...
public:
	...
	loan_document operator +(loan_document second_par);
	bool operator >(loan_document second_par);
	operator double();
	double operator--();
	double operator--(int second_par);
	void get_rezult();
	void get_double();
};
...
void loan_document::get_double(){
	printf("\nNew_Loan_sum = %lf\nNew_Loan_interest = %lf\n", loan_sum, loan_interest);
}

loan_document loan_document::operator+(loan_document second_par){
	loan_document temp("SUM", "Of procent", 0, 0, 0);
	
	temp.loan_sum = loan_sum + second_par.loan_sum;
	temp.loan_interest = loan_interest + second_par.loan_interest;
	
	return temp; 
}

bool loan_document::operator >(loan_document second_par){
	return loan_sum > second_par.loan_sum ? 1 : 0; 
}

loan_document::operator double(){
	return loan_sum + loan_interest;
}

double loan_document::operator--(){
	loan_sum -= loan_interest;
	return loan_sum;
}

double loan_document::operator--(int second_par){
	loan_sum -= loan_interest;
	return loan_sum;
}
...
int main(){
	...
	loan_document b("Пупок", "Васькин", 1234567890, 125, 12.5);
	b.get_rezult();
	printf("\n");
	...
	printf("After transformation 'sum + interest' to (double): %lf\n", (double) b);
	printf("After transformation 'sum + interest' to (double): %s\n", b);
	printf("When we use overload 'sum' (>): ");
	if(b > a){
		printf("First");
	}
	else 
	...
	printf ("\nPostfics decrementation loan_sum: %lf\n",b--);
	printf ("Prefics decrementation loan_sum: %lf\n\n",--b);
}
